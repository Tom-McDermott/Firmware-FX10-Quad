...
/*
 * LVDS/LVCMOS PHY configuration.
 *
 * QUAD RECEIVER RETARGET (interface-contract-RESOLVED.md §1.2, §3, §4):
 * The non-loopback (#else) branch is retargeted from the base example's
 * differential LVDS to the Quad board's 32-bit single-ended LVCMOS DDR
 * interface, clock-slaved to the FPGA's 125 MHz P0CLK/P1CLK:
 *   - modeSelect  = LVCMOS   (single-ended; the board is not differential LVDS)
 *   - wideLink=1 + dataBusWidth = LVCMOS 32 lanes (P1D[15:0] + P0D[15:0])
 *   - gearingRatio = 2:1     (DDR: data on both clock edges)
 *   - clkSrc = LVCMOS_IF     (external interface clock supplied by the FPGA)
 *   - lvcmosClkMode = CLK_SLAVE
 * This mirrors the SDK's known-good 32-bit LVCMOS DDR widelink config
 * (mtw/UVC_UAC_4K_CAMERA/cy_gpif_header_lvcmos.h) and selects the driver's
 * "LVCMOS DDR Widelink + link training" path (cy_lvds.c ~L1447), which trains
 * against linkTrainingPattern 0x125A4B78 == Tom's 0x784B5A12 byte-reversed.
 *
 * NEEDS HARDWARE / CONFIGURATOR VALIDATION:
 *  - The 125 MHz DDR interface clock has no dedicated SDK enum (LVCMOS options
 *    are 160/100/80 MHz). In clock-slave mode interfaceClock is a nominal hint
 *    (the real clock arrives on the pin), so 160 MHz is used per the reference;
 *    interfaceClock_kHz carries the true 125000.
 *  - ctrlBusBitMap is left 0 (no control-bus handshake). If the FPGA also trains
 *    control inputs (contract §4.2) or the GPIF needs address/control lines,
 *    regenerate cy_lvds_gpif0_* + ctrlBusBitMap via the ModusToolbox LVDS
 *    configurator for this exact geometry.
 *  - The loopback (#if LVDS_LB_EN) branch is unmaintained for the Quad board.
 */
cy_stc_lvds_phy_config_t cy_lvds_phy0_config = {
    ...
    .gearingRatio        = CY_LVDS_PHY_GEAR_RATIO_2_1,       /* DDR (both edges) */
    .clkSrc              = CY_LVDS_GPIF_CLOCK_LVCMOS_IF,     /* clock from FPGA  */
    .clkDivider          = CY_LVDS_GPIF_CLOCK_DIV_INVALID,
    .loopbackModeEn      = false,
    .isPutLoopbackMode   = false,
    .interfaceClock      = CY_LVDS_PHY_INTERFACE_CLK_160_MHZ, /* nominal; slave  */
    .interfaceClock_kHz  = 125000UL,                          /* actual FPGA clk */
    .wideLink            = 1u,
